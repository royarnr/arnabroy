...
public class PerfectoDriverFactory {
	...
	/*
	 * ******************************************
	 * ****Changed method arguments based on persona***
	 */
	public static WebDriver getPerfectoRemoteWebDriver(
			MobileExecutionPlatform executionPlatform, String deviceName,
			String perfectoURL, Browser browser, String persona) {
		...
		try {
			if(!"".equals(persona))
			{
				desiredCapabilities.setCapability("", persona);
			}
			
		} catch (Exception e) {
			// TODO: handle exception
			System.out.println("No persona invoked");
		}
		try {
			switch (executionPlatform) {
